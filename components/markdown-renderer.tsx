...
const importStyle = (style: string) =>
  dynamic(() => import("react-syntax-highlighter/dist/esm/styles/prism").then((mod) => mod[style]), { ssr: false })
    ...
    <div className="prose max-w-none dark:prose-invert">
      <ReactMarkdown
        ...
        components={{
          ...
          img({ node, ...props }) {
            return <img {...props} className="rounded-md max-w-full h-auto" loading="lazy" />
          },
          table({ node, ...props }) {
            return (
          ...
          th({ node, ...props }) {
            return <th {...props} className="border px-4 py-2 bg-gray-100 dark:bg-gray-800" />
          },
          td({ node, ...props }) {
